        ...
        <table align="center" border="3" cellspacing="0" class="table">
            ...
            <c:forEach var="list" items="${requestScope.teacherList}" varStatus="status">
                <tr align="center">
                    ...
                    <td>${list.username}</td>
                    <td>${list.position}</td>
                    <td><a href="ShowCourseServlet?id=${list.id}&name=${list.username}">查看</a></td>
                    <td>${list.detail}</td>
                    <td><a href="alterTeacher.jsp?id=${list.id}&username=${list.username}&position=${list.position}&detail=${list.detail}&password=${list.password}">修改</a>&nbsp;
                        <a onclick="javascript:return del()" href="UpdateTeacherServlet?id=${list.id}"style="color: red">删除</a><td>
                </tr>

            </c:forEach>
        </table>
    </div>
    <div align="center"><a href="alterTeacher.jsp" class="btn btn-primary" >+新增教师</a></div>
