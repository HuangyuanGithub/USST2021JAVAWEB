...
<main>
    ...
    <form method="post" action="UpdateCourseServlet">
        <div align="center">
            ...
            <h3>请输入课程的信息:<br><br>
                名称：<input type="text" name="name" value="${param.name}" required="required"><br><br>

                学院：<select name="collegeName">
                <c:forEach var="list" items="${requestScope.collegeList}" varStatus="status">
                    <option value="${list.name}"<c:if test="${list.name==param.collegeName}" > selected="selected"</c:if>>${list.name}</option>
                </c:forEach>
                </select>
